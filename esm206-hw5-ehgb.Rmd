...
salamanders_counted_og <- salamanders %>%
  filter(section == "OG") %>% 
  count(year, section)

salamanders_counted_cc <- salamanders %>% 
  filter(section == "CC") %>% 
  count(year, section)
...
salamander_line <- ggplot(data = salamanders_counted,
  ...
  geom_line(data = salamanders_counted_og,
            aes(x = year, y = count)) +
  geom_line(data = salamanders_counted_cc, 
            aes(x = year, y = count)) +
  theme_bw()
...
#salamander_beeswarm <- beeswarm(length1 ~ section, data = salamanders)

#salamander_beeswarm <- ggplot(data = salamanders, aes(x = section, y = length1)) + ggbeeswarm::geom_beeswarm()
...
salamander_hist <- ggplot()+
  geom_density(data = salamander_og,
               aes(x = length1),
               fill = "orange",
               alpha = 0.5) +
  geom_density(data = salamander_cc,
               aes(x = length1),
               fill = "red",
               alpha = 0.5) +
  theme_bw() +
  ...
  geom_vline(data = salamander_og,
             aes(xintercept = mean(length1)),
             color = "orange",
             linetype = "dashed",
             size = 1,
             alpha = 0.75) +
  geom_vline(data = salamander_cc,
             aes(xintercept = mean(length1)),
             color = "red",
             linetype = "dashed",
             size = 1,
